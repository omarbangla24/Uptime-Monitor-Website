...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <h1 class="text-2xl font-semibold text-gray-800 mb-6">Create Subscription Plan</h1>
        ...
        <form action="{{ route('admin.subscriptions.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" id="websites_limit" name="websites_limit" value="{{ old('websites_limit', 1) }}" min="0" required
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                </div>
                <div>
                    <label for="checks_per_minute" class="block text-sm font-medium text-gray-700">Checks per Minute</label>
                    <input type="number" id="checks_per_minute" name="checks_per_minute" value="{{ old('checks_per_minute', 1) }}" min="1" required
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                </div>
                <div>
                    <label for="data_retention_days" class="block text-sm font-medium text-gray-700">Data Retention (days)</label>
                    <input type="number" id="data_retention_days" name="data_retention_days" value="{{ old('data_retention_days', 30) }}" min="1" required
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                </div>
                <div>
                    <label for="team_members" class="block text-sm font-medium text-gray-700">Team Members</label>
                    <input type="number" id="team_members" name="team_members" value="{{ old('team_members', 1) }}" min="1" required
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="features" name="features" rows="3"
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('features') }}</textarea>
                </div>
                <div class="md:col-span-2">
                    <div class="flex flex-wrap -mx-2">
                        <div class="flex items-center mx-2 mb-2">
                            <input id="ssl_monitoring" name="ssl_monitoring" type="checkbox" value="1"
                                   {{ old('ssl_monitoring') ? 'checked' : '' }}
                        ...
                        <div class="flex items-center mx-2 mb-2">
                            <input id="domain_expiry_monitoring" name="domain_expiry_monitoring" type="checkbox" value="1"
                                   {{ old('domain_expiry_monitoring') ? 'checked' : '' }}
                                   class="h-4 w-4 text-blue-600 border-gray-300 rounded">
